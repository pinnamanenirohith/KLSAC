...
export default async function HomePage() {
  const [events, storiesRes, settingsRes, newsRes, domainsRes, clubsRes, statsRes] = await Promise.all([
    ...
    supabase.from('news_articles').select('slug, title, excerpt, photo_url, category, date, featured').eq('featured', true).order('sort_order', { ascending: true }).order('date', { ascending: false }).limit(3),

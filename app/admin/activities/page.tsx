...
export default function ActivitiesAdminPage() {
  ...
  async function load(d: string) {
    setLoading(true);
    const url = d === 'all' ? '/api/admin/activities' : `/api/admin/activities?domain=${d}`;
    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm" style={{ color: '#71717A' }}>{activities.length} activities{domain !== 'all' ? ` in ${domain}` : ''}</p>
        </div>
